...
vunit_lib.add_source_files(join(root, "tb_top_sample_repr.vhd"))
...
# NOTE: Less than 2 parameters makes the whole system fail...
def gen_sample_repr_tests(obj, var1, var2):
    for var1, var2 in product(var1, var2):
        tb_cfg = dict(
            VAR1_PY=var1,
            VAR2_PY=var2
        )
...
tb_top_sample_repr = vunit_lib.test_bench("tb_top_sample_repr")
for test in tb_top_sample_repr.get_tests():
    # if test.name == "LiteBus - Modifying SPI config":
        gen_sample_repr_tests(test, ['0'], ['1'])
